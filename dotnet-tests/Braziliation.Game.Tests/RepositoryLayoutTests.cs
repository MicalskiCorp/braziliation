...
public sealed class RepositoryLayoutTests
{
    ...
    {
        var root = RepoRootFinder.FindRepositoryRoot();
        var path = Path.Combine(root, "Braziliation.sln");
        Assert.That(File.Exists(path), Is.True);

        var text = File.ReadAllText(path);
        Assert.That(text, Does.Contain("Assembly-CSharp"));
        Assert.That(text, Does.Contain("Assembly-CSharp.csproj"));
    }

    [Test]
    public void CiTestProject_Lives_Under_DotnetTests()
    {
        var root = RepoRootFinder.FindRepositoryRoot();
        var path = Path.Combine(root, "dotnet-tests", "Braziliation.Game.Tests", "Braziliation.Game.Tests.csproj");
        Assert.That(File.Exists(path), Is.True,
            "O projeto de testes para CI deve estar em dotnet-tests/ (fora de Tests/, que o Unity costuma usar).");
    }
}

# Criativo — Gestão Criativa do Braziliation

> Hub de gestão criativa do projeto. Centraliza lendas, narrativa, ideias e brainstorms.
> Alimenta o desenvolvimento de mapas, cenários, monstros e personagens.
> Agente responsável: `@GameCreative` (definição em `.claude/agents/game-creative.md`)
> Pendências: [`TODO.md`](TODO.md) · operações: skill [`gerir-todo`](../../.claude/skills/gerir-todo/SKILL.md)

## Navegação

| Seção | Descrição | Arquivo |
|-------|-----------|---------|
| [`Estados/`](Estados/index.md) | Cidades por estado — monstros, lugares, lendas, ideias | **Estados** |
| [`Historia/`](Historia/index.md) | Premissa, arcos narrativos e personagens | Historia |
| [`Lendas/`](Lendas/index.md) | Catálogo master de lendas brasileiras + mapeamento | Lendas |
| [`Ideias/`](Ideias/index.md) | Pool de ideias com tags e prioridade | Ideias |
| [`Brainstorm/`](Brainstorm/index.md) | Sessões de brainstorm por tema | Brainstorm |

## Como Usar

| Operação | Comando |
|----------|---------|
| Nova cidade | `@GameCreative Nova cidade: {Nome} — {Estado}` |
| Novo estado | `@GameCreative Novo estado: {Nome}` |
| Catalogar lenda | `@GameCreative Catalogar lenda: {Nome}` |
| Mapear lenda | `@GameCreative Mapear: {Lenda} → {monstro\|mapa\|cenário\|NPC}` |
| Registrar ideia | `@GameCreative Registrar ideia: {descrição}` |
| Brainstorm | `@GameCreative Brainstorm: {tema}` |
| Novo arco | `@GameCreative Novo arco: {nome}` |
| Novo personagem | `@GameCreative Novo personagem: {nome}` |
| Varredura criativa | `@GameCreative Varredura criativa` |

---

> Estilo do projeto: Plataforma 2D Pixel Art · Dieselpunk Pós-Apocalíptico Brasileiro
